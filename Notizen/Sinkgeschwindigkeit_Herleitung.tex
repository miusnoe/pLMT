...
Durch Einsetzen der Definitionen aus Sektion 1 in das Kräftegleichgewicht erhalten wir:
\begin{align*}
    F_G - F_A &= F_W \\
...
\begin{align*}
    c_W &= \frac{24}{\text{Re}} \quad &&\text{für } \text{Re} < 0{,}5 \quad \text{(laminare Strömung / Stokes-Bereich)} \\
    c_W &= \frac{18{,}5}{\text{Re}^{0{,}6}} \quad &&\text{für } 0{,}5 < \text{Re} < 500 \\
    c_W &= 0{,}44 \quad &&\text{für } \text{Re} > 500 \quad \text{(turbulente Strömung / Newton-Bereich)}
\end{align*}
...
\begin{equation*}
    v^2 = \frac{4 d g (\rho_T - \rho_F)}{3 \cdot \left(\dfrac{24 \cdot \eta_F}{\rho_F \cdot d \cdot v}\right) \cdot \rho_F} = \frac{4 d^2 g v (\rho_T - \rho_F)}{72 \cdot \eta_F}
\end{equation*}
