...
\begin{itemize}
\item[\color{miugray}$\blacktriangleright$] Berechnen Sie den mittleren Durchmesser des sich in der Zentrifuge einstellenden Flüssigkeitsringes.
\item[\color{miugray}$\blacktriangleright$] Was ist der Vorteil einer Dekanterzentrifuge gegenüber anderen Zentrifugentypen?
\end{itemize}
\begin{figure}
\centering
\begin{tikzpicture}[scale=0.7]
\draw[line width=0.01mm,fill=miugray] (0,0) circle (2cm);
\uncover<9->{\draw[miugray2, dashed ,fill=miugray] (0,0) circle (1.65cm);}
\draw[miugray2,fill=white] (0,0) circle (1.3cm);

\uncover<2->{\draw[thick] (-1.65,0) to (-3,0) node [left] {$40\%$};}
\draw (0,0) to (20:2cm) node [right] {$r_\mathsmaller{{a}}$};
\uncover<9->{\draw[miugray2,dashed] (0,0) to (-20:1.65cm) node [right,xshift=4mm,yshift=0mm] {$r_\mathsmaller{{m}}$};}
...
\pp
$c_W$ ist quasi eine Art Rohrreibungszahl nur halt eben für Kugeln und nicht für Rohre :)
