...
\draw[->] (0,0) to (0,4) node[right,xshift=1mm] {$p$};
\draw[-, rounded corners=5pt] (0.5,0.5) to (1,1) to (1,3);
\draw[-, uniblau] (1,1) to [bend right=35](3,3);
%\draw[<->, miugray2!60,dashed] (0.5,2) to (3.5,2);
%\draw[<->, miugray2!60,dashed] (1.5,2.5) to (1.5,0.5);
\node[scale=0.7] at(1.75,2.5) {\large flüssig};
\node[scale=0.7] at(3,1) {\large gasförmig};
\node[scale=0.7,rotate=90] at(0.5,2) {\large fest};
\end{tikzpicture}
...
\uncover<2->{
{\bfseries
$\rightarrow$ Lässt sich als ``Abkürzung'' nutzen, wenn man von Punkt A nach B kommen möchte! }}\pp 
Abgeleitet aus:
\begin{align*}
\frac{dp}{dT} = \frac{p \cdot \Delta h_{vM}}{R \cdot T^2}
\end{align*}
Annahmen:
\begin{align*}
pV = nRT \\
\Delta v = V_{Gas} - V_{Fl} \approx V_{Gas} \\
\Delta h_{vM} = \text{const.}
\end{align*}
...
\begin{align*}
\ln(\frac{p_2}{p_1}) &= \frac{\Delta h_{vM}}{R} \cdot \left( \frac{1}{T_1} - \frac{1}{T_2} \right) \\ 
\Delta h_{vM} &= \frac{\ln(\frac{p_2}{p_1}) \cdot R}{\left( \frac{1}{T_1} - \frac{1}{T_2} \right)} \\ 
\Delta h_{vM} &= \frac{\ln(\frac{p_2}{p_1}) \cdot R \cdot {\color{red}T_1 \cdot T_2}}{\left( \frac{1}{T_1} - \frac{1}{T_2} \right)\cdot {\color{red}T_1 \cdot T_2}} \\ 
\Delta h_{vM} &= \frac{\ln(\frac{p_2}{p_1}) \cdot R \cdot {\color{red}T_1 \cdot T_2}}{\left( \frac{{\color{red}T_1 \cdot T_2}}{T_1} - \frac{{\color{red}T_1 \cdot T_2}}{T_2} \right)}\\ 
\Delta h_{vM} &= \frac{\ln(\frac{p_2}{p_1}) \cdot R \cdot T_1 \cdot T_2}{T_2 - T_1} \\ 
\end{align*}
...
\begin{center}
\Large
Nächste Übung: \par \large Mischungsrechnungen eigenständig recherchieren
\end{center}
...
\begin{frame}
\textbf{D-Wert}: Wie lange muss ich erhitzen um die Keimzahl um 1/10 zu reduzieren?
\pp
\textbf{z-Wert}: Wie viel höher muss die Temperatur sein, damit der Prozess 10-mal so schnell abläuft?
\pp 
\textbf{F-Wert}: Die Summe aller einzelnen Erhitzungsschritte
