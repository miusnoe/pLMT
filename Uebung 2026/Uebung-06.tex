...
\antwort{
\begin{align*}
D_{1\rightarrow 2} = \frac{4 ~\text{min}}{\log (\frac{10^6}{1,1 \cdot 10^5})} = 4,17 ~\text{min}
\end{align*}
}
\else
\fi
...
\marginnote[]{\small Der \fplmt{z-Wert} gibt an, wie stark die temperaturabhängige Änderung des D-Werts ist. Ein niedriger z-Wert bedeutet, dass die Änderung durch die Temperatur sehr stark ist.}
...
\begin{align*}
10^{-9} ~(\text{Sporen} /\text{g}) \cdot 500 ~\text{g/Packung} = \frac{1}{2.000.000} ~\text{Sporen/Packung}
\end{align*}
...
\begin{align*}
\frac{1}{3,16 \cdot 10^{-9}} = 3,16 \cdot 10^{9}
\end{align*}
}
\else
\fi
...
%####################### RAUSGENOMMEN FÜR ALS ZUSATZAUFGABE
%
