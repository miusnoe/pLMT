...
b) &&v_3 &= \frac{20\cm^2 \cdot 3\mskip3mu \cm/\text{s}}{25\mskip3mu \cm^2} &&=2,4\cm/\text{s}
\end{align*}
}
\pagebreak
\else
...
\begin{itemize}
\item Welcher relativer Unterdruck wird an der engsten Stelle erreicht? 
...
\end{align*}
Jenachdem, wie man es umstellt, kann man auch auf ein negatives Ergebnis kommen. Beides ist richtig.
...
Wobei $p_2$ der Druck an der engsten Stelle und $p_1$ der Druck am Ende des Rohres ist.
\par
...
&h_a &&= \frac{V}{A_1} = \frac{10\mskip3mu \m^3}{4,52~\text{m}^2} &&= 2,21\mskip3mu \m
\end{align*}
...
Einsetzen:
\begin{align*}
t &= \sqrt{\frac{2}{g}} \cdot \frac{4,52\mskip3mu \m^2}{0,005\mskip3mu \m^2} \cdot \sqrt{2,21\mskip3mu \m} \\
&= 604 ~\text{s}
\end{align*}
}
...
\ifshowme
\antwort{Die Entleerungszeit verhält sich umgekehrt proportional zur Querschnittsfläche des Ausflusses, während sie proportional zur Füllhöhe der Wurzel ist. \pp 
...
Einsetzen:
\begin{align*}
&\text{a)} && \frac{999,8\mskip3mu \kg/\m^3 \cdot 0,3\mskip3mu \m \cdot 0,05\mskip3mu \m/\s}{1.790 \cdot 10^{-6}\mskip3mu \text{P} \cdot \s} &&\approx 8.378 \\
&\text{b)} && \frac{999,8\mskip3mu \kg/\m^3 \cdot 0,3\mskip3mu \m \cdot 0,01\mskip3mu \m/\s}{1.790 \cdot 10^{-6}\mskip3mu \text{P} \cdot \s} &&\approx 1.676 \\
&\text{c)} && \frac{999,8\mskip3mu \kg/\m^3 \cdot 0,1\mskip3mu \m \cdot 0,05\mskip3mu \m/\s}{1.790 \cdot 10^{-6}\mskip3mu \text{P} \cdot \s} &&\approx 2.793 \\
\end{align*}
...
\begin{itemize}
\item Wie viele Liter Nitrat-Dünger sind bis dahin ausgelaufen, wenn der Kanister einen Durchmesser von 1 m hat? 
\end{itemize} 
...
&\approx 1,236~\text{m}
\end{align*}
\marginnote[]{\fplmt{Anmerkung:} Hier habe ich beim Einsetzen der Werte nicht die Fläche A genommen, sondern das Verhältnis der Durchmesser zum Quadrat (ähnlich wie in Aufgabe 2)}[-2cm]
\pp
...
\begin{tikzpicture}[
    ...
    % 1. Das Hauptrohr
    \draw[double=white, double distance=14pt, rounded corners=20pt, thick] 
        (1, 8) -- (1, 10) -- (3, 10) -- (3, -2) -- (4.5, -2) -- (4.5, -0.5) -- (6, -0.5);

    % 2. Öffnung oben links
    \draw[thick] (0.7, 8) -- (1.3, 8);
    \draw[thick] (0.73, 8.1) -- (1.27, 8.1); 
    
    %
%    \draw[thick,uniblau,fill=uniblau, -latex, scale=2] (1,8) -- (1,7.5);

    % 3. Muffe / Rohrsegment für das Messgerät (weiter nach unten versetzt)
    \draw[thick] (2.7, 1.0) -- (3.3, 1.0);
    \draw[thick] (2.7, 2.0) -- (3.3, 2.0);

    % 4. Das Messgerät (Druckanzeige, nach unten verschoben)
    \draw[thick] (3.25, 1.5) -- (4.0, 1.5); % Verbindungsstück
    \draw[thick, fill=white] (4.6, 1.5) circle (0.7); % Das Ziffernblatt
    \draw[thick] (4.6, 1.5) -- (5.0, 1.0); % Die Nadel
    \draw[thick] (5.15, 1.55) -- (5.25, 1.55); % Kleine Markierung im Display

    % 5. Der Hahn / das Ventil rechts ("Erdnuss"-Griff)
    \draw[thick, fill=white] (6, -0.2) rectangle (6.2, -0.8); % Anschluss/Muffe
    \draw[thick, fill=white] (6.2, -0.3) rectangle (7.2, -0.7); % Rohrstück
    \draw[thick] (6.8, -0.3) -- (6.8, 0.15); % Ventilstift
    
    % Erdnuss-förmiger Flügelgriff
    \draw[thick, fill=white] 
        (6.5, 0.25) 
        .. controls (6.5, 0.45) and (6.7, 0.45) .. (6.8, 0.25) 
        .. controls (6.9, 0.45) and (7.1, 0.45) .. (7.1, 0.25) 
        .. controls (7.1, 0.05) and (6.9, 0.05) .. (6.8, 0.25) 
        .. controls (6.7, 0.05) and (6.5, 0.05) .. cycle;

    % 6. Bemaßung h1 (weiter nach rechts verschoben)
    \draw[thick] (-0.8, -2.35) -- (-0.8, 8);
    \draw[thick] (-1.0, -2.35) -- (-0.6, -2.35); % Unterer Strich
    \draw[thick] (-1.0, 8) -- (-0.6, 8);         % Oberer Strich
    \node[left] at (-0.9, 2.8) {\Huge $h_1$};

    % 7. Bemaßung h2 (weiter nach rechts und weiter nach unten)
    \draw[thick,xshift=4pt] (0.4, -2.35) -- (0.4, 1.7);
    \draw[thick,xshift=4pt] (0.2, -2.35) -- (0.6, -2.35);   % Unterer Strich
    \draw[thick,xshift=4pt] (0.2, 1.7) -- (0.6, 1.7);       % Oberer Strich
    \node[left,xshift=4pt] at (0.3, -0.2) {\Huge $h_2$};
...
\begin{align*}
v_1 = \frac{\dot{V}}{A_1} = \frac{5 \cdot 10^{-4} ~\text{m$^3$/s}}{1,24 \cdot 10^{-4} ~\text{m}^2} = 4,03 ~\text{m/s}
\end{align*}
...
Nach $v_2$ umstellen:
\begin{align*}
v_2 &= \sqrt{\frac{2 (p_1 - p_2)}{\rho} + 2g(h_1 - h_2) + v_1^2} \\
&= \sqrt{\frac{2 (2-1~\text{bar})}{1.000~\text{kg/m$^3$}} + 2 \cdot 9,81 ~\text{m/s}^2 \cdot (1-6~\text{m}) + (4,03~\text{m/s})^2} \\
